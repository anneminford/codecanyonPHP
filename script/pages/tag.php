...
$quotes->additional_join("
		...
		AND `associations` . `type` = 0
		AND `associations` . `target_id` = {$tag->tag_id}
	");

/* Make it so it will display only the active and the quotes which are not private */
$quotes->additional_where("AND `quotes` . `active` = '1' AND `associations` . `target_id` = {$tag->tag_id}");
...
$quotes->display_pagination('tag/' . $tag->url);
?>
